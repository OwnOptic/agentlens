...
/**
 * Standard result for a tool that has not been implemented yet.
 *
 * IMPLEMENTATION NOTE: delete the call to this helper when you implement a tool,
 * but keep returning `notConnected(...)` for the case where credentials or
 * permissions are genuinely missing at runtime.
 */
export function notImplemented(toolName: string, sources: DataSource[], howToImplement: string): ToolResult<never> {
  return notConnected(
    `The ${toolName} tool is not implemented yet in this deployment, so no data is available. No figures can be reported.`,
    sources.map((source) => ({
      source,
      status: 'not_connected' as const,
      detail: 'Tool not implemented in this build.',
    })),
    howToImplement,
  );
}
