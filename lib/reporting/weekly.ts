    ...
    const bodyHtml = s.body
      ...
      .map((line) => {
        if (line.startsWith('|')) {
          ...
          if (isSep) return '';
          const tag = 'td';
          return `<tr>${cells.map((c) => `<${tag} style="padding:4px 8px;border:1px solid #ddd">${c.trim()}</${tag}>`).join('')}</tr>`;
        }
...
<div style="margin-bottom:24px">
  <h2 style="font-size:16px;color:#1f2937;border-bottom:2px solid #10b981;padding-bottom:4px">${s.title}</h2>
...
<body style="font-family:system-ui,sans-serif;max-width:800px;margin:0 auto;padding:24px;background:#fff;color:#111">
  <h1 style="font-size:22px;color:#065f46">${subject}</h1>
  <p style="color:#6b7280;font-size:13px">Period: ${period}</p>
  <hr style="border:none;border-top:1px solid #e5e7eb;margin:16px 0">
  ${sections.map(wrapSection).join('\n')}
  <hr style="border:none;border-top:1px solid #e5e7eb;margin:16px 0">
  <p style="font-size:11px;color:#9ca3af">Generated by AgentLens v2 &bull; ${new Date().toISOString()}</p>
</body>
</html>`;
}
